use gpui::{ReadGlobal, div, prelude::*};
use show::{
    Show,
    assets::{AssetId, Sequence},
};

use super::{Pool, PoolDelegate};

pub struct SequencePool {}

impl SequencePool {
    pub fn new() -> Self {
        Self {}
    }
}

impl PoolDelegate for SequencePool {
    type Item = Sequence;

    fn title(&self, _cx: &mut Context<Pool<Self>>) -> &str {
        "Sequences"
    }

    fn render_cell_content(
        &mut self,
        asset_id: AssetId<Self::Item>,
        _w: &mut gpui::Window,
        cx: &mut Context<Pool<Self>>,
    ) -> Option<impl IntoElement> {
        let Some(sequence) = Show::global(cx).assets.sequences.get(&asset_id).map(|eg| eg.read(cx))
        else {
            return None;
        };

        Some(
            div()
                .h_full()
                .flex()
                .flex_col()
                .justify_center()
                .text_center()
                .child(sequence.label.clone()),
        )
    }
}
