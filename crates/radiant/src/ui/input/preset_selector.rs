...
impl PresetSelector {
    fn handle_on_click(
        ...
        AppState::update_global(cx, |state, cx| {
            state.open_preset_selector_window(selector, window, cx);
        });
    }
}
...
impl PresetSelectorWindow {
    pub fn new(
        ...
        let dimmer_table_view = cx.new(|cx| Table::new(dimmer_table, "dimmer-table", window, cx));

        let position_table = AssetTable::new(Show::global(cx).assets.position_presets.clone());
        let position_table_view =
            cx.new(|cx| Table::new(position_table, "position-table", window, cx));

        let gobo_table = AssetTable::new(Show::global(cx).assets.gobo_presets.clone());
        let gobo_table_view = cx.new(|cx| Table::new(gobo_table, "gobo-table", window, cx));

        let color_table = AssetTable::new(Show::global(cx).assets.color_presets.clone());
        let color_table_view = cx.new(|cx| Table::new(color_table, "color-table", window, cx));

        let beam_table = AssetTable::new(Show::global(cx).assets.beam_presets.clone());
        let beam_table_view = cx.new(|cx| Table::new(beam_table, "beam-table", window, cx));

        let focus_table = AssetTable::new(Show::global(cx).assets.focus_presets.clone());
        let focus_table_view = cx.new(|cx| Table::new(focus_table, "focus-table", window, cx));

        let control_table = AssetTable::new(Show::global(cx).assets.control_presets.clone());
        let control_table_view =
            cx.new(|cx| Table::new(control_table, "control-table", window, cx));

        let shapers_table = AssetTable::new(Show::global(cx).assets.shapers_presets.clone());
        let shapers_table_view =
            cx.new(|cx| Table::new(shapers_table, "shapers-table", window, cx));

        let video_table = AssetTable::new(Show::global(cx).assets.video_presets.clone());
        let video_table_view = cx.new(|cx| Table::new(video_table, "video-table", window, cx));
