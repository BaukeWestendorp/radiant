...
pub const TRAFFIC_LIGHT_WIDTH: Pixels = px(69.0);
...
pub fn titlebar(window: &Window, cx: &App) -> Stateful<Div> {
    ...
    div()
        ...
        .pl(TRAFFIC_LIGHT_WIDTH + TRAFFIC_LIGHT_POSITION.x)
        ...
        .flex()
        .items_center()
        .child(div().font_weight(FontWeight::BOLD).pb(px(-3.0)).child(window.window_title()))
