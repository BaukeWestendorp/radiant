...
pub type UniverseData = arrayvec::ArrayVec<Slot, MAX_UNIVERSE_SIZE>;

impl Universe {
    /// Creates a new universe with the given number.
    pub fn new(number: UniverseNumber) -> Self {
        Universe { number, start_code_slot: 0, data_slots: arrayvec::ArrayVec::new() }
    }
    ...
    pub fn with_start_code(number: UniverseNumber, start_code: Slot) -> Self {
        Universe { number, start_code_slot: start_code, data_slots: arrayvec::ArrayVec::new() }
    }

    /// Returns the start code slot and data slots.
    pub fn slots(&self) -> arrayvec::ArrayVec<Slot, { 1 + MAX_UNIVERSE_SIZE }> {
