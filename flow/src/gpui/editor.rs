    ...
    Def::Control: VisualControl<Def>,
{
    ...
    pub fn graph<'a>(&self, cx: &'a App) -> &'a Model<Graph<Def>> {
    ...
    fn open_node_context_menu(&mut self, cx: &mut Context<Self>) {
    ...
    fn close_node_context_menu(&mut self, _: &CloseNodeContextMenu, cx: &mut Context<Self>) {
        self.new_node_context_menu.update(cx, |menu, cx| {
            menu.hide(cx);
        });
    }

    fn handle_drag_move(&mut self, _: &DragMoveEvent<()>, cx: &mut Context<Self>) {
    ...
    fn handle_mouse_up(&mut self, _: &MouseUpEvent, _cx: &mut Context<Self>) {
...
where
    ...
    Def::NodeData: VisualNodeData,
{
    pub fn build(graph: Model<Graph<Def>>, cx: &mut Context<Self>) -> View<Self> {
        cx.new_view(|cx| {
    ...
    pub fn hide<View: 'static>(&mut self, cx: &mut Context<View>) {
        self.shown = false;
        cx.notify();
    }
    ...
    fn filtered_nodes(&self, cx: &App) -> Vec<Def::NodeKind> {
        Def::NodeKind::all()
            .filter(|n| {
                n.label()
                    .to_lowercase()
    ...
    fn filter<'a>(&self, cx: &'a App) -> &'a str {
    ...
    fn render_node_list(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let render_list_item = move |item: AnyElement, cx: &Context<Self>| -> AnyElement {
            div()
                .p_1()
