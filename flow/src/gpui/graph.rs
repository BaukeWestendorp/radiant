    ...
    Def::Control: VisualControl<Def>,
{
    ...
    fn emit_should_save_event(&mut self, event: &GraphEvent<Def>, cx: &mut App) {
        match &event {
            GraphEvent::AddNode { .. }
    ...
    fn handle_socket_event(&mut self, event: &SocketEvent, cx: &mut Context<Self>) {
        ...
        match event {
            ...
            SocketEvent::EndNewEdge => {
                let update_graph = |output_id, input_id, cx: &mut Context<Self>| {
                    self.graph.update(cx, |graph, cx| {
    ...
    fn render_edges(&self, cx: &Context<Self>) -> Div {
    ...
    fn render_new_edge(&self, cx: &Context<Self>) -> Div {
    ...
    fn get_socket_position(&self, node_id: NodeId, socket: &Parameter, cx: &App) -> Point<Pixels> {
